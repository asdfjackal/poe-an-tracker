...
function App() {
  ...
  const [inventory, updateInventory] = useState(
    savedInventory === null
      ? new Map<string, number>(Object.keys(data).map((key) => [key, 0]))
      : new Map<string, number>(Object.entries(JSON.parse(savedInventory)))
  );
  ...
  const increment = (key: string) => {
    ...
    localStorage.setItem(
      "archnemesis-inventory",
      JSON.stringify(Object.fromEntries(newInventory.entries()))
    );
  };
